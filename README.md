# AI Reading Tools: Instant Dictionary and Summarizer for Google Chrome and Microsoft Edge

AI Reading Tools is a plugin for Google Chrome and Microsoft Edge that can have a transformative effect on your reading and language learning. I originally built it for my personal use, out of frustration with exisiting dictionaries. These are the benefits of the "Dictionary" function of this plugin over more traditional dictionaries:
...
The extension can also give you a short and straightforward summary of the current tab, either by choosing the "Summarize" option from the extension's menu, or by simply pressing `Ctrl + Shift + K`.

![Demo](demo-summarize.gif)
...
3. Acquire an API key from an OpenAI-compatible provider. By default the extension calls [OpenAI](https://api.openai.com/v1).
4. Select some word(s)/text, right click, and choose "Define Selection". You can also press the combination "Ctrl + Shift + L" to achieve the same function. As this is your first time calling the dictionary, a dialog will open asking you to enter your API key. Enter the API key you acquired in the previous step, and you are all set!
5. To summarize the current page instead of defining a selection, press "Ctrl + Shift + K".

The extension sends one OpenAI-compatible `/chat/completions` request per lookup. I personally use the [OpenRouter](https://openrouter.ai/api/v1) endpoint along with `google/gemini-2.5-flash-lite` for the quickest results. You can set your own custom endpoints and models via the extension's menu under `Advanced API Options`.
